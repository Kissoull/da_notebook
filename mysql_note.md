...
## 基本概念
1. 列(column): 字段
2. 行(row): 表种的一个记录(数据库记录, record)
3. 主键(primary key): 数据唯一标志列(任何时候要为每个表创建一个主键)
4. 子句(clause): 
...
## 预览查询
- DESCRIBE(简写为: DESC): 查看表结构

## SQL语法(语句, 子句, 关键字/操作符/字段/谓词, 通配符)
- SELECT: 查询语句
	- 子句: 
		- DISTINCT: 去重
		- CONCAT(): 字段(concatenate), 拼接(sql使用'+'或'||')
		- AS: 关键字(key word), 取别名(也叫导出列, derived column)
	- FROM: 选择数据表
	- ORDER BY: 排序(就近优先排序)
		- DESC: 关键字(key word), 降序(只对指定列降序)
		- ASC: 关键字(key word), 升序(default)
	- WHERE: 过滤条件(filter condition)
		- BETWEEN AND: 关键字(key word), 范围查询
		- IS NULL: 关键字(key word), 空值检查
		- AND: 操作符(operator), 优先级大于OR
		- OR: 操作符(operator), 优先级小于AND
		- IN: 操作符(operator), 指定范围
		- LIKE: 谓词(predicate), 模糊搜索
			- 通配符(wildcard): 
				- '%': 任何字符出现任意次数, 匹配0个以上字符
				- '_': 匹配单个字符
		- REGEXP: 正则表达式
			...
			- '\\': 转义(escaping)(\\. \\- \\\ e.t.)
			- '\\'引用元字符: '\\f': 换页 '\\n': 换行 '\\r': 回车 '\\t': 制表 '\\v': 纵向制表
		- BINARY: 关键字(key word), 正则表达式匹配大小写
	- GROUP BY: 分组
	- HAVING: 聚合(**where在数据分组前过滤, having在数据分组后过滤**; where过滤行, having过滤分组)
- LIMIT: 限制输出
